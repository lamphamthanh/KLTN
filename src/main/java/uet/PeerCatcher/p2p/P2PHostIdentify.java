    ...
    public static class P2PHostDetectionReducer extends Reducer<Text, Text, Text, Text> {
        ...
        protected void reduce(Text key, Iterable<Text> values, Context context)
            ...
            for (Text i : values) {
                ...
                String[] sets = tmp[0].split("\\.");
                //System.out.println(sets[0] + " " + sets[1]);
                distinct16Set.add(sets[0] + "." + sets[1]);
                cache.add(tmp[0]);
                Integer src = Integer.valueOf(tmp[1]);
                Integer dst = Integer.valueOf(tmp[2]);
                if(!(src <= 1023 || dst <= 1023)) {
                    p2p_port += 1;
                }
                all += 1;
            ...
            double rate = p2p_port / all;
            if (distinct16Set.size() >= p2PHostDetectionThreshold ||
                    (distinct16Set.size() >= 10 && rate > 0.95)) {
                flag = 1;
            }
